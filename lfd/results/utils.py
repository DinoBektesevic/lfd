...
@contextmanager
def session_scope():
    """Return a transactional scope around a series of operations."""
    from . import Session
...
def __pprintEvents(events, short=True, digits=2):
    ...
    ----------
    events : `iterable`
        Iterable of Event objects to print in table-like format
    short : `bool`
        When True a short form table is printed, when False a long form table
        is printed.
    digits : `int`
...
def __pprintFrames(frames):
    """Prints a list of frames in a table-like format. Table format is:
            run camcol filter field ISO-time

    Parameters
    ----------
    frames : `iterable`
        Iterable of Frame objects to print in table-like format
    """
...
def pprint(objlist, *args, **kwargs):
    ...
    Events print the full frame identifier, time and coordinates of the object,
    when Frames are provided only the frame identifiers and time is printed.
    Long format prints all coordinates, while short format prints only 'frame'
    coordiantes of Events.

    Parameters
    ----------
    objlist : `iterable`
        Iterable of Frame or Event objects to print in table-like format
    short : `bool`, optional
        When True a short form table is printed, when False a long form table
        is printed. Only applies to Events.
    digits : `int`
        Number of significant digits printed for Events.
    """
