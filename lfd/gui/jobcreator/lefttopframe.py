...
class TopFrame(ttk.Frame):
    ...
    def __init__(self, parent, row=0, col=0):
        ...
        title = ttk.Label(self, text="Parameter options",
        ...
        #######################################################################
        numjobsl = ttk.Label(self, text="Num. Jobs: ")
        numjobsl.grid(row=row+1, column=col, pady=5, sticky=W)

        self.numjobs = ttk.Entry(self)
        ...
        #                    WALLCLOCK SELECTOR
        #######################################################################
        wallclockl = ttk.Label(self, text="Wallclock: ")
        wallclockl.grid(row=row+3, column=col, pady=3, sticky=W)

        self.wallclock = ttk.Entry(self)
        ...
        #                    CPUTIME SELECTOR
        #######################################################################
        cputimel = ttk.Label(self, text="Cputime: ")
        cputimel.grid(row=row+4, column=col, pady=3, sticky=W)

        self.cputime = ttk.Entry(self)
        ...
        #                    PPN SELECTOR
        #######################################################################
        ppnl = ttk.Label(self, text="Processors per node: ")
        ...
        queuedm = ttk.OptionMenu(self, self.ppn, *queueoptions)
        queuedm.grid(row=row+5, column=col+1, pady=5, sticky=W+E)

        #######################################################################
        #                    COMMAND SELECTOR
        #######################################################################
        commandl = ttk.Label(self, text="Command: python -c \n \"import detecttrails as dt;")
    ...
    def getn(self):
        """Reads the current value from the ttk.Entry for number of jobs and
        ...
        if n == 0:
            messagebox.showerror("Input Error", "Number of jobs can't be 0")
        else:
            return n
