...
from lfd.createjobs import writer
...
class Jobs:
    ...
    save_path : `str`
        ...
        ``~/Desktop/createjobs``
    res_path : `str`
        Path to subdirectory on cluster master where results will be copied
        once the job is finished.
    template_path : `str`
        Path to the desired template
    template : `str`
        Full template text as a string
    queue : `str`
        QSUB queue type: serial, standard or parallel. Defaults to
        standard. Your local QSUB setup will limit wallclock, cputime and queue
        name differently than assumed here.
    wallclock : `str`
        Maximum wallclock time allowed for a job in hours. Default: 24:00:00
    cputime : `str`
        Maximum cpuclock time allowed for a job in hours. Default: 48:00:00
    ppn : `str`
    ...
    **kwargs : `dict`
    ...
    def __init__(self, n, runs=None, template_path=None, save_path=None,
                 ...
                 command=('python3 -c "import detecttrails as dt; dt.DetectTrails($).process()"\n'),
    ...
    def __init__template(self, template_path=None, template=None):
        ...
        template : `str` or `None`, optional
            Entire template given as a single big string.
        """
    ...
    def _findKwargs(self):
        ...
        if isinstance(self.runs, list) and \
           (isinstance(self.runs[0], Event) or isinstance(self.runs[0], Frame)):
            self.pick = "Results"
    ...
    def create(self):
        ...
        if not self.runs:
            print("There are no runs to create jobs from. Creating jobs for all "
                  "runs in runlist.par file.")
        ...
        print("Creating: \n "
              f"    {njobs} jobs with {nruns} runs per job \n"
              f"    Queue:     {self.queue} \n"
              f"    Wallclock: {self.wallclock} \n"
