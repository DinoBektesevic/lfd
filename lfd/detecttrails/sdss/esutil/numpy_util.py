...
class ArrayWriter:
    ...
    def simple_write(self, arrin, **keys):
        ...
        if header == True or misc.isstring(header):
            if header==True:
    ...
    def fancy_write(self, arrin, **keys):
        ...
        for i in xrange(len(fields)): 
            n=fields[i]
            pname=printnames[i]
            header += forms[n] % misc.center_text(pname,max_lens[n])

        if title is not None:
            title = misc.center_text(title, len(header))
