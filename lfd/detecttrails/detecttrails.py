...
order of operations on the targeted data to succesfully run LFD. The module
also handles the IO operations required to succesfully store results and wraps
the required functionality for easier debugging.
"""
import os
import bz2
import traceback
...
import cv2
from cv2 import RETR_LIST, RETR_EXTERNAL, RETR_CCOMP, RETR_TREE  # noqa: F401
from cv2 import (CHAIN_APPROX_NONE,  # noqa: F401
                 CHAIN_APPROX_SIMPLE,
...
from lfd.detecttrails.processfield import (process_field_bright,
                                           process_field_dim,
                                           setup_debug)
...
def process_field(results, errors, run, camcol, filter, field, params_bright,
    ...
    try:
        ...
        if detection:
            results.write(printit + f"{res['x1']} {res['y1']} {res['x2']} {res['y2']}\n")
        else:
...
class DetectTrails:
    ...
    def __init__(self, **kwargs):
        ...
        self.params_dim = {
            ...
            "lwTresh": 5,
            "thetaTresh": 0.15,
            "erodeKernel": _np.ones((3, 3), _np.uint8),
    ...
    def _load(self):
        ...
        * field (full specification run-filter-camcol-frame)
        ...
        if 'filter' in kwargs:
            ...
            if (self._camcol != 0) and (self._run != 0):
                self._pick = 'run-camcol-filter'

        if 'filter' not in kwargs:
            if (self._field != 0) and (self._camcol != 0):
                self._pick = 'camcol-frame'

        if (self._field != 0) and (self._camcol != 0) and (self._filter != "0"):
            self._pick = 'field'
    ...
    def process(self):
        ...
        """
        with open(self.results, "a") as results, open(self.errors, "a") as errors:
