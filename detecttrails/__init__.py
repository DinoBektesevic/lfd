...
* BOSS - the 'boss' directory which usually contains both 'photo' and 'photoObj'
    directories
* BOSS_PHOTOOBJ - if the 'photoObj' directory is stored in a different position
    instead of BOSS this can be overwritten to point to that location. In
    combination with PHOTO_REDUX, can be used to almost completely customize the
    data layout.
* PHOTO_REDUX - if the 'photo' directory is not in BOSS this should point to its
    location instead. The required files are 'photoRunAll' and 'runList' which
    should live inside the sub-directory 'redux'.
...
The detection algorthm can be used to run on other sources of data given that
the images are in FITS format and that the catalog for each image is also given
as a header table of a FITS file. There should be one such catalog fits per image
and it should follow the naming convention adopted by SDSS (i.e. 'photoObj...').
Other catalog sources can also be used but not implemented. See removestars
module on several examples of old deprecated catalog functionality and how other
source catalogs could be adopted and used instead of SDSS catalog.
...
DetectTrails(run= , camcol={1-6}, field={all}, filter={ugriz})
    ...
    * Detection of dim linear features.
    Each detection step is individually configurable through the 'params_<step>'
    dictionary of detection/execution parameters. For example, changing detection
    parameters:
    ...
    their explanation. A special 'debug' can be set to True for any of the
    ...
    ################
      ...
      ----------------- 
          ...
          minAreaRectMinLen: 1
              Once contours have been returned minimal area rectangles are fitted.
              Length of its sides are determined. If either side is shorter than
              minAreaRectMinLen value, in pixels, the detection is dismissed.
          lwTresh: 5
          ...
          nlinesInSet: 3
              ...
              voted nlinesInSet number of lines are taken into consideration for
              further consistency checks. This is called a "set" of lines because
              one set is fitted to the raw image and the other to the rectangles
          ...
          debug: False
              verboose output of various steps including saving image snapshots:
              1) equBRIGHT      - equalized 8bit int image without known objects
              2) dilateBRIGHT   - equalized and dilated image
              3) contoursBRIGHT - rects. that pass the tests drawn on image
              4) boxhoughBRIGHT - rects. with corresponding fitted lines (set 1)
      ...
      ----------------- 
          ...
          dro: 20 (see above)
          debug: False
              The output is similar to debug BRIGHT option with additional saved
              snapshots:
      ...
      ----------------------
          ...
          defaultxy: 20 (in pixels)
            Half of the length of the side of a square that is drawn over stars.
            Used if there is no petro. radius or if the length/2 of square side
            is larger than maxxy.
          maxxy: 60 (in pixels)
          ...
          filter_caps: {'u': 22, 'g': 22,'r': 22, 'i':22,'z': 22},
            stars on the image that are dimmer than filter caps will not be removed.
          magcount: 3
    ...
    For an more detailed algorithm see process_field_dim and process_field_bright
    from process_field module.
...
def setup(bosspath=BOSS, photoobjpath=BOSS_PHOTOOBJ, photoreduxpath=PHOTO_REDUX,
    ...
    """Sets up the required environmental paths for detecttrails module. If only
    BOSS is supplied it is assumed that SDSS conventions are followed and that
    the folder structure resembles:
    boss
      /photo
        /redux
          photoRunAll-dr##.par
    ...
    if all(photoobjpath, photoreduxpath):
        # Set these paths manually on your own responsibility
        PHOTO_REDUX   = os.path.join(BOSS, "photo/redux")
        BOSS_PHOTOOBJ = os.path.join(BOSS, "photoObj")
    if debugpath:
        DEBUG_PATH = os.path.abspath(os.path.curdir)
